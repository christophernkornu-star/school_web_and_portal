...
const PerformanceChart = dynamic(() => import('@/components/PerformanceChart'), {
  ssr: false,
  loading: () => (
    <div className="h-[260px] sm:h-[300px] md:h-[340px] w-full flex items-center justify-center bg-slate-50/80 dark:bg-slate-800/50 animate-pulse rounded-2xl sm:rounded-3xl" />
  )
})
...
export default function TeacherDashboard() {
  ...
  useEffect(() => {
    async function fetchTermProgress() {
      ...
      if (termRes.data) {
        ...
        const progress = Math.min(Math.max(Math.round((daysPassed / (totalDays || 1)) * 100), 0), 100)
  ...
  const currentTerm = dashboardData?.currentTerm || 'Active Term'
    ...
    <div className="min-h-screen bg-slate-50/50 dark:bg-slate-900 pb-16 sm:pb-20 font-sans text-slate-900 dark:text-slate-100 transition-colors">
      <div className="max-w-7xl mx-auto px-3.5 sm:px-6 lg:px-8 space-y-5 sm:space-y-6 lg:space-y-8 pt-3.5 sm:pt-6">
            ...
            <div className="space-y-2.5 sm:space-y-4 max-w-2xl min-w-0">
              <div className="space-y-1 sm:space-y-1.5">
                ...
                <p className="text-blue-200/90 text-xs sm:text-sm md:text-base flex items-center gap-2 font-medium flex-wrap">
                  ...
                  Ready to manage attendance, continuous assessments, and class records?
                </p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 pt-1">
                ...
                <span className="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1 rounded-xl text-[11px] sm:text-xs font-bold bg-amber-400/20 border border-amber-400/30 text-amber-300 shadow-2xs">
                  <span className="truncate max-w-[140px] sm:max-w-none">{currentTerm}</span>
                </span>
              </div>
            </div>
              ...
              <div className="bg-white/10 backdrop-blur-xl p-3.5 sm:p-5 rounded-xl sm:rounded-2xl border border-white/15 shadow-inner flex md:flex-col justify-between items-center md:items-start gap-2">
                ...
                <div className="flex items-baseline gap-2">
                  ...
                  <span className="text-[10px] sm:text-[11px] text-emerald-300 font-bold flex items-center">
                    <TrendingUp className="w-3 h-3 mr-0.5 shrink-0" /> Active
                  </span>
                </div>
              </div>
            </div>
        ...
        <section className="bg-white dark:bg-slate-800/80 rounded-2xl sm:rounded-3xl shadow-xs border border-slate-200/80 dark:border-slate-700/80 overflow-hidden">
          {/* Header with Methodist Blue Background */}
          <div className="px-4 sm:px-6 py-3.5 sm:py-4 bg-[#003B5C] border-b border-[#002a42] flex items-center justify-between">
            <h2 className="text-xs sm:text-sm font-black uppercase tracking-wider text-white flex items-center gap-2">
              ...
              <span>Core Teaching Operations</span>
            </h2>
            <span className="text-[11px] sm:text-xs text-blue-200/80 font-medium hidden sm:inline">Daily Workflow Shortcuts</span>
          </div>
          ...
          <div className="p-3.5 sm:p-5 md:p-6">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3.5 md:gap-4">
              ...
              <QuickActionCard 
                ...
                title="Manage Scores" 
                description="Continuous marks & exams"
                icon={ClipboardList} 
              />
              <QuickActionCard 
                href="/teacher/assessments"
                title="Online Assessment" 
                description="Quizzes & homework"
                icon={FileText} 
              />
              <QuickActionCard 
                href="/teacher/reports"
                title="Class Reports" 
                description="Cards & broadsheets"
                icon={BarChart3} 
              />
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 sm:gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-2 space-y-5 sm:space-y-6 lg:space-y-8 min-w-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 md:gap-5">
              <div 
                ...
                className="cursor-pointer bg-white dark:bg-slate-800/80 rounded-2xl sm:rounded-3xl p-3.5 sm:p-5 border border-slate-200/80 dark:border-slate-700/80 shadow-xs hover:shadow-md hover:border-[#003B5C]/40 dark:hover:border-blue-500/40 transition-all duration-200 flex items-center justify-between group active:scale-[0.98]"
              >
                <div className="flex items-center gap-3 sm:gap-3.5 min-w-0">
                  <div className="p-2 sm:p-2.5 bg-[#003B5C]/10 text-[#003B5C] dark:bg-blue-500/20 dark:text-blue-300 rounded-xl sm:rounded-2xl shrink-0 group-hover:scale-105 transition-transform">
                  ...
                  <div className="min-w-0">
                    ...
                    <Tooltip content={assignedClasses.length > 0 ? assignedClasses.join(', ') : 'No cohorts assigned'}>
                      <h3 className="text-lg sm:text-xl md:text-2xl font-black text-slate-900 dark:text-white truncate">
                ...
                <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-slate-50 dark:bg-slate-700/50 flex items-center justify-center text-slate-400 group-hover:text-[#003B5C] dark:group-hover:text-blue-400 transition-colors shrink-0 ml-2">
            ...
            <section className="bg-white dark:bg-slate-800/80 rounded-2xl sm:rounded-3xl shadow-xs border border-slate-200/80 dark:border-slate-700/80 overflow-hidden">
              <div className="px-4 sm:px-6 py-3.5 sm:py-4 bg-[#003B5C] border-b border-[#002a42] flex items-center justify-between">
                <h2 className="text-xs sm:text-sm font-black uppercase tracking-wider text-white flex items-center gap-2">
                  <span className="w-1.5 h-4 bg-amber-400 rounded-full shrink-0" />
                  <span>Class Academic Trend Across Terms</span>
                </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-5 sm:gap-6 lg:gap-8 min-w-0">
            ...
            <section className="bg-white dark:bg-slate-800/80 rounded-2xl sm:rounded-3xl shadow-xs border border-slate-200/80 dark:border-slate-700/80 overflow-hidden flex flex-col justify-between">
              <div>
                {/* Header with Methodist Blue Background */}
                <div className="px-4 sm:px-5 py-3.5 sm:py-4 bg-[#003B5C] border-b border-[#002a42] flex items-center justify-between">
                  ...
                  <Link 
                    ...
                    className="text-xs font-bold text-amber-300 hover:text-amber-200 hover:underline"
                  >
                    View All
                  </Link>
                </div>
                      ...
                      {announcements.map((announcement) => (
                        <div key={announcement.id} className="py-3 first:pt-0 last:pb-0 space-y-1.5">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <span className="text-[10px] sm:text-[11px] font-semibold text-slate-400 font-mono">
            ...
            {/* Profile & Specialization Summary Card */}
            <section className="bg-white dark:bg-slate-800/80 rounded-2xl sm:rounded-3xl shadow-xs border border-slate-200/80 dark:border-slate-700/80 overflow-hidden flex flex-col justify-between">
              <div>
                {/* Header with Methodist Blue Background */}
                <div className="p-4 sm:p-5 bg-[#003B5C] border-b border-[#002a42] flex items-center gap-3.5">
                  <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl sm:rounded-2xl bg-white/10 backdrop-blur-md text-amber-300 border border-white/20 flex items-center justify-center font-black text-xs sm:text-sm shadow-inner shrink-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] sm:text-[11px] font-mono text-blue-200/90 truncate mt-0.5">
                ...
                <div className="p-4 sm:p-5 space-y-3">
                  <div className="grid grid-cols-2 gap-2.5">
                    <div className="p-2.5 sm:p-3 rounded-xl sm:rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-700/80 flex flex-col justify-between">
                  ...
                  <div className="p-2.5 sm:p-3 rounded-xl sm:rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-700/80 space-y-1">
...
function DashboardSkeleton() {
  return (
    <div className="max-w-7xl mx-auto px-3.5 sm:px-6 lg:px-8 py-5 space-y-6">
      ...
      <Skeleton className="h-32 sm:h-36 w-full rounded-2xl sm:rounded-3xl" />
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <Skeleton className="h-72 sm:h-80 w-full rounded-2xl sm:rounded-3xl" />
        </div>
        <div className="space-y-6">
          <Skeleton className="h-60 sm:h-64 rounded-2xl sm:rounded-3xl" />
          <Skeleton className="h-60 sm:h-64 rounded-2xl sm:rounded-3xl" />
        </div>
      </div>
    </div>
