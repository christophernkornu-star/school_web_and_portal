...
export default function MockExamsPage() {
  ...
  // The roster actually shown/scored for the currently selected mock.
  const [mockRoster, setMockRoster] = useState<Student[]>([])
  ...
  async function loadClassDetails(classId: string) {
    ...
    // 3. Get Mocks for this class (hide mocks where all recorded students have graduated)
    ...
    if (mocksData && mocksData.length > 0) {
      ...
      const { data: scoreRows } = await supabase
        .from('mock_scores')
        .select('mock_exam_id, students(status)')
        .in('mock_exam_id', mockIds)

      // A mock "retires" from the live picker once every student who sat it has
      // since graduated — it's done, and belongs on Admin > Historical Reports >
      // Mock Results instead. A brand-new mock with no scores yet always stays
      // visible so it can actually be used.
      const hasAnyScorer = new Set<string>()
      const hasActiveScorer = new Set<string>()
      ;(scoreRows || []).forEach((row: any) => {
        hasAnyScorer.add(row.mock_exam_id)
        if (row.students?.status && row.students.status !== 'graduated') {
          hasActiveScorer.add(row.mock_exam_id)
        }
      })
      setMocks(mocksData.filter((m: any) => !hasAnyScorer.has(m.id) || hasActiveScorer.has(m.id)))
    } else {
      setMocks(mocksData || [])
    }
  }
  ...
  async function loadMockScores(mockId: string) {
    ...
    const isCurrentMock = mockData?.academic_year === currentAcademicYear

    const { data, error } = await supabase
      .from('mock_scores')
      .select('student_id, subject_id, score, students(id, first_name, middle_name, last_name, gender, status)')
      .eq('mock_exam_id', mockId)

    if (error) {
      toast.error('Failed to load mock scores: ' + error.message)
      return
    }

    const loadedScores: Record<string, Record<string, string>> = {}
    const scoredStudents = new Map<string, Student>()
    ...
    ;(data || []).forEach((item: any) => {
      ...
      if (st && !scoredStudents.has(item.student_id)) {
        scoredStudents.set(item.student_id, {
          id: st.id,
    ...
    if (isCurrentMock) {
      ...
      students.forEach(s => merged.set(s.id, s))
      // Graduated students are deliberately left off the LIVE mock page — once a
      // student graduates, their mock record "moves" to the admin's
      // Historical Reports > Mock Results tab instead of cluttering the current roster.
      scoredStudents.forEach((s, id) => {
        if (!merged.has(id) && s.status !== 'graduated') merged.set(id, s)
      })
      roster = Array.from(merged.values())
      students.forEach(s => { if (!loadedScores[s.id]) loadedScores[s.id] = {} })
    } else {
      roster = Array.from(scoredStudents.values()).filter(s => s.status !== 'graduated')
    }
