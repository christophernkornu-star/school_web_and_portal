    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 font-sans text-gray-900 dark:text-gray-100 transition-colors">
        ...
        {showSheet && selectedMock && currentMockData && (
             <div className="bg-white text-black min-h-screen">
                 ...
                 <div className="max-w-[297mm] mx-auto p-4 md:p-8 print:p-0">
                     <div className="text-center mb-6 border-b-2 border-black pb-4">
                         <h1 className="text-2xl font-bold font-serif uppercase tracking-wide">Biriwa Methodist "C" Basic School</h1>
                     ...
                     <table className="w-full border-collapse border border-black text-xs md:text-sm">
                         <thead>
                             <tr className="bg-gray-100">
                                 <th className="border border-black p-2 w-10">SN</th>
                                 <th className="border border-black p-2 text-left">NAME</th>
                                 {subjects.map(s => (
                                     <th key={s.id} className="border border-black p-2 w-12 text-center rotate-heads">
                                         {getShortSubjectName(s.name)}
                                     </th>
                                 ))}
                                 <th className="border border-black p-2 w-16 bg-gray-50">AVG</th>
                                 <th className="border border-black p-2 w-16 bg-gray-200">AGG</th>
                                 <th className="border border-black p-2 w-12 text-center">POS</th>
                             </tr>
                         </thead>
                         <tbody>
                             {processedData.map((student, idx) => (
                                 <tr key={student.id}>
                                     ...
                                     <td className="border border-black p-2 font-medium uppercase whitespace-nowrap px-3">{student.fullname}</td>
                                     {subjects.map(s => (
                                         <td key={s.id} className="border border-black p-2 text-center">
                                             {student.scores[s.id] || '-'}
                                         </td>
                                     ))}
                                     <td className="border border-black p-2 text-center font-bold">{student.average.toFixed(1)}</td>
                                     <td className="border border-black p-2 text-center font-bold bg-gray-100">{student.aggregate}</td>
                                     <td className="border border-black p-2 text-center font-bold">{student.position}</td>
                                 </tr>
                             ))}
                         </tbody>
                     </table>
