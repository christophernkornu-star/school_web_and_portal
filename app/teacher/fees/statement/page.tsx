...
function ClassStatementContent() {
  ...
  useEffect(() => {
    if (!classId || !academicYear || !termId) {
  ...
  const loadData = async () => {
    try {
      if (!classId || !academicYear || !termId) return
      ...
      if (termData) {
        ...
        setYearLabel(termData.academic_year || academicYear)
      } else {
        setYearLabel(academicYear)
      }
      ...
      // 4. Get Fee Structures for selected year and term
