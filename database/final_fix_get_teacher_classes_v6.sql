-- Final Fix v6: Perfected Security for JHS Subject Assignments
-- This solves the bug where "BOOL_OR(can_edit)" caused JHS teachers
-- with 1 subject to accidentally get "can_edit_all_subjects" = true.
...
BEGIN
  ...
  SELECT DISTINCT
    ...
    -- STRICT PERMISSION LOGIC (The Root Cause Fix)
    -- If they are in a class_teacher level, they get edit_all_subjects = true.
    -- If they are in JHS, this will evaluate to FALSE.
  ...
  -- Join purely for permission aggregation (retained for backward compatibility on joins)
